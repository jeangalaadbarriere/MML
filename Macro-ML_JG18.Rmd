...
The three-pass estimator of $\boldsymbol\gamma_g$ is the product of the estimates of
$\boldsymbol{\tilde{\Theta}}$ and $\boldsymbol{\tilde{\gamma}}$.
...
the computation of the $t_{stat}$. They found an asymptotic estimator of the
Covariance matrix, which can be estimated with the following:
...
Where, if we define by $\tilde{\xi}$ the matrix storing every $\boldsymbol{\tilde{\xi_t}}$ for $0 < t < T+1 $, we get 
...
$\hat{\gamma_g}$ are however quite far from theirs. But since we know we do not have the same PCA as we've seen, although there have the same magnitude, except for the component.
...
$\boldsymbol{r_t} = \beta'.\boldsymbol{\gamma} + \beta'.\boldsymbol{\xi_t} + \boldsymbol{\epsilon_t}, (1)$ giving
$\boldsymbol{r_t} = \tilde{\beta}'\boldsymbol{\tilde{\gamma}} + \tilde\beta'\boldsymbol{\tilde\xi_t} + \boldsymbol{\epsilon_t}, (1bis)$
and $\boldsymbol{\bar{g_t}} = \Theta'.\boldsymbol{\xi_t} + \boldsymbol{\zeta_t}, (2)$ giving
$\boldsymbol{\bar{g_t}} = \tilde{\Theta}'\boldsymbol{\tilde\xi_t} + \zeta_t, (2bis)$,
where $\boldsymbol{\bar{g_t}} = \boldsymbol{g_t} - \boldsymbol{\delta}$. The meaning of the first equation is simply that we extracted a new base $(\boldsymbol{\tilde\xi_t})_{t<T+1}$ with a PCA on the $(r_t)_{i}$, and now we express the $(r_t)_i$ in the firsts components of this regression.
...
$(\boldsymbol{\tilde\xi_t})_{t<T+1}$. $\Theta$ is then the base change matrix. But
this has nothing evident: it only makes sense if we consider, either that the $(g_t)_i$ are very closed to each other, either that the
"true" function linking the $(g_t)_i$ with the excess returns $(r_t)_t$
is linear (insofar as the base $(\boldsymbol{\tilde\xi_t})_{t<T+1}$ is built on linear combinations of $(r_t)_i$).
...
write the new equation $(2')$ this way: $\boldsymbol{\bar{g_t}} = \mathcal{F}(\boldsymbol{\xi_t}) + \boldsymbol{z^*_t}$ with $\boldsymbol{z^*_t}$ the new residual.
...
"Train a VAGM function on
$(\boldsymbol{\bar{g_t}})_{t}$ and
...
(the default value if we authorize it) on each $(\tilde{\xi_t}_i)_{t<T+1}$, whereas we do not authorize it in the non-smooth one.
...
close to the ones we obtained with the previous three-steps methodology, specially in the non-smooth regression. It enforces us in the idea that this method can work, although we don't have a lot of data to train our VGAM (T=678).

Evaluating the efficiency of our new algorithm is more difficult. We
cannot express the $t_{stat}$ as previously, because the formula we used for the covariances worked only under linearity assumptions. We could
...
precise. And since $\gamma_g$ is a product of estimates, it seems that we cannot obtain an easy estimate of the $t_{stat}$ this time.
...
kable(df, caption = "Comparaison of the differents methods for the third-step, with the Conventional PCA. 1: 3-step methodology, 2: Not-smooth VGAM, 3: smooth VGAM, gamma: gamma_g, MR2: Mean Square Error", row.names = F)
```

We also apply this extension to the sparse PCA. Once again, we see that
our smooth VGAM method performs better in term of MSR, when the not-smooth VGAM method gives almost the same result as the original
...
kable(df, caption = "Comparaison of the differents methods for the third-step, with the Sparse PCA. 1: 3-step methodology, 2: Not-smooth VGAM, 3: smooth VGAM, gamma: gamma_g, MR2: Mean Square Error", row.names = F)
```
