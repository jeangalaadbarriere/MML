...
The three-pass estimator of $\boldsymbol\gamma_g$ is the product of
...
the computation of the t-stat. They found an asymptotic estimator of the
...
Where
$\hat{\Sigma}^v = \boldsymbol{\tilde\xi_t} \boldsymbol{\tilde\xi_t}$ and
$Mat^T = (\hat{\gamma}^T . (\hat{\Sigma}^v)^{-1} \otimes I_d)$

Here, $\hat{\Pi}_{21} = \hat{\Pi}_{12}^T$, and $\hat{\Pi}_{11}$,
$\hat{\Pi}_{12}$ and $\hat{\Pi}_{22}$ are the HAC estimators of Newey
and West (1987). They can account for heterogeneity and autocorrelation
standard error.
...
$\hat{\gamma_g}$ are however quite far from theirs.
...
$r_t = \beta'.\gamma + \beta'.\xi_t + \epsilon_t, (1)$ giving
$r_t = \tilde{\beta}'\tilde{\gamma} + \tilde\beta'\boldsymbol{\tilde\xi_t} + \epsilon_t, (1bis)$
and $\bar{g_t} = \Theta'.\xi_t + \zeta_t, (2)$ giving
$\bar{g_t} = \tilde{\Theta}'\boldsymbol{\tilde\xi_t} + \zeta_t, (2bis)$,
where $\bar{g_t} = g_t - \delta$. The meaning of the first equation is
simply that we extracted a new base $\boldsymbol{\tilde\xi_t}$ with a
PCA on the $(r_t)_{i}$, and now we express the $(r_t)_i$ in the firsts
components of this regression.
...
$\boldsymbol{\tilde\xi_t}$. $\Theta$ is then the base change matrix. But
this has nothing evident: it only makes sense if we consider, either
that the $(g_t)_i$ are very closed from one another, either that the
"true" function linking the $(g_t)_i$ with the excess returns $(r_t)_t$
is linear (insofar as the base $\boldsymbol{\tilde\xi_t}$ is built on
linear combinations of $(r_t)_i$).
...
$\bar{g_t} = \mathcal{F}(\xi_t) + z^*_t$ with $z^*_t$ the new residual.
This leads to the equation we will use:
$$\bar{g_t} = \tilde{\mathcal{F}}(\tilde{\xi_t}) + z^*_t (2bis')$$

In this new model, the desired $\gamma_g$ is now equal to
$\bar{\gamma_g} = \tilde{\mathcal{F}}(\tilde{\gamma})$.
...
$\boldsymbol g_t - \tilde{\Theta}'\boldsymbol{\tilde\xi_t}$ and
$\boldsymbol{\tilde\xi_t}$ to obtain
...
(the default value if we authorize it) on each $\tilde{\xi_t}$, whereas
we do not authorize it in the non-smooth one.
...
close to the ones of the original article, and to the ones we computed
in the previous sections, specially in the non-smooth regression. It
enforces us in the idea that this method can work, although we don't
have a lot of data to train our VGAM (T=678).

Evaluating the efficiency of our new algorithm is more difficult. We
cannot express the t-stat as previously, because the formula we used for
the covariances worked only under linearity assumptions. We could
...
precise. And since $\gamma_g$ is a product, it seems that we cannot
obtain an easy estimate of the t-stat this time.
...
as in the original three-step meyhodology, while the ones of the smooth
