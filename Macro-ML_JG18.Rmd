...
is the product of two estimators ($\hat{\Theta}$ and $\hat{\gamma}$).
...
Where, if we define by $\tilde{\xi}$ the matrix storing every
...
$\hat{\gamma_g}$ are however quite far from theirs. But since we know we
do not have the same PCA as we've seen, although there have the same
magnitude, except for the component.
...
$(\boldsymbol{\tilde\xi_t})_{t}$ to obtain
...
lineralise our $\hat{\mathcal{F}}_{VGAM}$ to obtain an estimate of
...
little amount of data we have. Therefore, the good MSR we obtain with
the smooth VGAM can either express a truth: the model is non linear and
we need to smooth the VGAM. Or it can just be overfitting. To solve
