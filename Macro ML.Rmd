...
R_1 <- R
R_d_1 <- t(R_d)
r_pca_1 <- PCA(R_d_1, ncp=15,graph = F)
pc <- r_pca_1$ind$coord
ar1 <- VAR(pc, p=1)
ksi <- residuals(ar1)
...
ksi<-ts(ksi)
lm1 <- tslm(R_1~0+ksi)
beta1 <- t(coefficients(lm1))

R_1_bar <- colMeans(R_1)
lm2 <- lm(R_1_bar~0+beta1)
gamma <- coefficients(lm2)
```
...
df <- data.frame(Factor = paste0("PC",1:9),gamma_g = gamma_g)
