...
```{r}
pca_R <- PCA(R_d, ncp=15,graph = F)

# factors are the innovations to the PCs :
ar1 <- VAR(pca_R$ind$coord, p=1)
ksi <- residuals(ar1)

 ####ksi <- r_pca_1$ind$coord
...
Pb : time-regressions de r ou de r demeaned? Je pense que c'est r demeaned (mais ça équivaut normalement à régresser r avec constante)
```{r}
R <- ts(R[-1,])
###R <- ts(R) 
...
ksi<-ts(ksi)
lm1 <- tslm(R~ksi)
beta <- t(coefficients(lm1))[,-1]

R_bar <- colMeans(R)
lm2 <- lm(R_bar~beta)
gamma <- coefficients(lm2)[-1]
```
...
```
Bof, pas terrible. Peut-être faut-il normaliser les résidus de la PCA sur les macrovariables?
