...
In the second step, we run a time-series regressions of r on $\Xi$ to estimate $\beta$. We can then run a cross-sectional regression of $\bar{r}$ on the columns of $\beta$ to estimate $\gamma$.
...
STEP 3. Run time-series regressions of g on $\Xi$ to estimate theta
In the final third step we run a time-series regression of $g$ on $\Xi$ to estimate $\Theta$.
...
And now, we can estimate $\gamma_g$ as the product of $\hat{\Theta}}$ and $\hat{\gamma}$.
...
$$Mat = (\hat{\gamma}^T . (\hat{\Sigma}^v)^{-1} \otimes I_d)$$
And here, $\hat{\Pi}_{21} = \hat{\Pi}_{12}^T$, and $\hat{\Pi}_{11}$, $\hat{\Pi}_{12}$ and $\hat{\Pi}_{22}$ are the HAC estimators of Newey and West (1987). They can account for heterogeneity and autocorrellation standard error.
